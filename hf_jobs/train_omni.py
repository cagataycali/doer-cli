...
tags: [doer, omni, lora, multimodal]
datasets: [{args.dataset}]
---

# doer Omni — {args.model} fine-tune (text + image + audio)

- base: `{args.model}`
- method: LoRA r={args.lora_r} (merged)
- records: {len(train_ds)} train ({n_text} text, {n_img} image, {n_aud} audio)
- steps: {args.iters}

## Use

```bash
DOER_PROVIDER=transformers DOER_MODEL={args.push_to} \\
  do --img x.png --audio y.wav "describe"
```
""")
